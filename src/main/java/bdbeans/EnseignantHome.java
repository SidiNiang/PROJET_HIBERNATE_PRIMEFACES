...
public class EnseignantHome {
	...
	public Enseignant findById(bdbeans.EnseignantId id) {
		logger.log(Level.INFO, "getting Enseignant instance with id: " + id);
		try {
			Enseignant instance = (Enseignant) sessionFactory.getCurrentSession().get("bdbeans.Enseignant", id);
			if (instance == null) {
	...
	public List findByExample(Enseignant instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("bdbeans.Enseignant")

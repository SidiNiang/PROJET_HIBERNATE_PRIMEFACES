...
public class PersonneHome {
	...
	public Personne findById(int id) {
		...
		try {
			Personne instance = (Personne) sessionFactory.getCurrentSession().get("bdbeans.Personne", id);
			if (instance == null) {
	...
	public List findByExample(Personne instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("bdbeans.Personne")
	...
	public Personne connexion(String login) {
	    ...
	    try {
	        Personne instance = (Personne) sessionFactory.getCurrentSession()
	                .createQuery("FROM Personne WHERE login = :login")
	                .setParameter("login", login)
	                .uniqueResult();
	        if (instance == null) {
	            logger.log(Level.INFO, "No user found with this login");
	        } else {

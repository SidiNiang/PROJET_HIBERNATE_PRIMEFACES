...
public class EtudiantHome {
	...
	public Etudiant findById(bdbeans.EtudiantId id) {
		logger.log(Level.INFO, "getting Etudiant instance with id: " + id);
		try {
			Etudiant instance = (Etudiant) sessionFactory.getCurrentSession().get("bdbeans.Etudiant", id);
			if (instance == null) {
	...
	public List findByExample(Etudiant instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("bdbeans.Etudiant")
